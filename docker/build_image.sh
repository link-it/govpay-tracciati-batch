...
Sorgente:
-v <VERSIONE>  : Imposta la versione della release binaria da utilizzare per il build (default: ${LATEST_GOVPAY_TRACCIATI_RELEASE})
...
LATEST_LINK="$(curl -qw '%{redirect_url}\n' https://github.com/link-it/govpay-tracciati-batch/releases/latest 2> /dev/null)"
LATEST_GOVPAY_TRACCIATI_RELEASE="${LATEST_LINK##*/}"
...
DOCKERBUILD_OPT=()
DOCKERBUILD_OPTS=(${DOCKERBUILD_OPTS[@]} '--build-arg' "govpay_tracciati_fullversion=${VER:-${LATEST_GOVPAY_TRACCIATI_RELEASE}}")
[ -n "${TEMPLATE}" ] &&  cp -f "${TEMPLATE}" buildcontext/commons/
[ -n "${CUSTOM_GOVPAY_HOME}" ] && DOCKERBUILD_OPTS=(${DOCKERBUILD_OPTS[@]} '--build-arg' "govpay_tracciati_home=${CUSTOM_GOVPAY_HOME}")
[ -n "${CUSTOM_GOVPAY_LOG}" ] && DOCKERBUILD_OPTS=(${DOCKERBUILD_OPTS[@]} '--build-arg' "govpay_tracciati_log=${CUSTOM_GOVPAY_LOG}")
if [ -n "${CUSTOM_RUNTIME}" ]
then
...
if [ -n "${LOCALFILE}" ]
then
  DOCKERFILE="govpay-tracciati/Dockerfile.daFile"
  cp -f "${LOCALFILE}" buildcontext/
else
  DOCKERFILE="govpay-tracciati/Dockerfile.github"
fi
...
if [ -z "$TAG" ] 
then
    REPO=${REGISTRY_PREFIX}/govpay-tracciati-batch
  TAGNAME=${VER:-${LATEST_GOVPAY_TRACCIATI_RELEASE}}
  TAG="${REPO}:${TAGNAME}"  
fi
